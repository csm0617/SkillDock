...
- 多选：点击技能行即切换选中；选中项的 `/name` token 同步进草稿；取消选中则移除对应 token。
...
**写入规则**：维护草稿中 `/name` token 的集合与顺序；选中即追加 `/name `，取消即删除该 token 及其后随空格；不触碰用户手输的其它文本。所有写入以 `draftRev` 为准，rev 变化时重新计算，避免覆盖用户正在进行的编辑。
